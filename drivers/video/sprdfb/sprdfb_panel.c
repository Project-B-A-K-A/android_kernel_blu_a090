...
static bool panel_check(struct panel_cfg *cfg)
{
	...
	//panel type not sure untill load config,should not check
	return rval;
#endif
...
static int panel_mount(struct sprdfb_device *dev, struct panel_spec *panel)
{
	...
	{
		case SPRDFB_PANEL_TYPE_MCU:
			panel->if_ctrl = &sprdfb_mcu_ctrl;
			break;
		case SPRDFB_PANEL_TYPE_RGB:
			panel->if_ctrl = &sprdfb_rgb_ctrl;
			break;
		case SPRDFB_PANEL_TYPE_MIPI:
			panel->if_ctrl = &sprdfb_mipi_ctrl;
			break;
		default:
			printk("sprdfb_udc_lcd: [%s]",__FUNCTION__);
			panel->if_ctrl = NULL;
			break;
	};

	if(panel->if_ctrl->panel_if_check){
		panel->if_ctrl->panel_if_check(panel);
	}
#endif	
...
static struct panel_spec *adapt_panel_from_uboot(uint16_t dev_id)
{
	...
	list_for_each_entry(cfg, panel_list, list) {
		...
		if  ( cfg->lcd_id == UDC_LCM_ID ){
			if  ( udc_lcd_create(SEC_LCD0, cfg) ){
				strncpy(zyt_peripherals_LCD_name, cfg->lcd_name,sizeof(zyt_peripherals_LCD_name)-1);
				printk(KERN_INFO "UDC LCD Panel(%s) 0x%x is attached!\n", cfg->lcd_name, cfg->lcd_id);
...
void esd_reset_panel_and_dispc(struct sprdfb_device *dev)
{
    #if defined(ZCFG_LCD_ESD_ENHANCE)/*@add by wangming for faq1690*/
    unsigned int save_addr = dispc_read(DISPC_OSD_BASE_ADDR);
    up(&dev->refresh_lock);
    dev->enable=0;
    sprdfb_panel_suspend(dev);

    dev->ctrl->resume(dev);
    dispc_write(save_addr,DISPC_OSD_BASE_ADDR);
