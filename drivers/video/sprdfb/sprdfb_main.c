...
#ifdef CONFIG_FB_ESD_SUPPORT
static int autotest_mode = 0; 
static int __init early_mode_check(char *str) 
{ 
	if (strncmp(str, "autotest", 7) == 0) 
		autotest_mode = 1; 
	printk("%s autotest_mode=%d\n", __func__,autotest_mode); 
	return 0; 
} 

early_param("androidboot.mode", early_mode_check); 
#endif
static int sprdfb_probe(struct platform_device *pdev)
{
	...
	dev->of_dev = &(pdev->dev);
	#ifdef CONFIG_UDC_LCD	
	dev->dev_id = udc->board->config.lcd_cs;	
	#else
	...
	if((dev->panel->ops->panel_esd_check!=NULL)  && /* @20141203:add jinq */
		(0 == autotest_mode))	/* Alex.shi add @20150918 for Autotest,let lcd test pass */
	{
	...
	if(SPRDFB_PANEL_IF_DPI == dev->panel_if_type){
		...
		pr_debug("sprdfb: No ESD panel_esd_check function !\n");
#endif
