...
unsigned int get_tsp_queue_count(void)
{
...
MTV_TS_PKT_INFO * mtv_peek_tsp(void)
{
...
static int mtv_create_tsp_pool(void)
{
	...
	for (i = 0; i < MAX_NUM_TS_PKT_BUF; i++) {
		tsp = (MTV_TS_PKT_INFO *)kmalloc(sizeof(MTV_TS_PKT_INFO), GFP_KERNEL|GFP_DMA);
		if (tsp == NULL) {
...
unlock_read:
	mtv_free_tsp(tsp); /* for "goto unlock_read;" statement. */
...
#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	mtv_disable_irq(mtv_cb_ptr->irq);
	mtv_delete_thread();
#endif
...
static int mtv_power_on(E_DMB_TV_MODE_TYPE tv_mode, unsigned long arg)
{
	...
	mtv_cb_ptr->isr_workqueue = create_singlethread_workqueue(RAONTV_DEV_NAME);
	...
	mtv_enable_irq(mtv_cb_ptr->irq); /* After DMB init */
#endif
...
static int mtv_close(struct inode *inode, struct file *filp)
{
	...
	atomic_set(&mtv_cb_ptr->open_flag, 0);
...
static int mtv_open(struct inode *inode, struct file *filp)
{
	...
	if (atomic_cmpxchg(&mtv_cb_ptr->open_flag, 0, 1)) {
...
static int mtv_init_device(void)
{
	...
	if ((ret=i2c_add_driver(&mtv_i2c_driver)) < 0) {
		DMBERR("RAONTV I2C driver register failed\n");
		goto err;
	}
	
#elif defined(RTV_IF_SPI)
	if ((ret=spi_register_driver(&mtv_spi_driver)) < 0) {
		DMBERR("RAONTV SPI driver register failed\n");
		goto err;
	}
#endif

#if defined(RTV_IF_SPI) || defined(RTV_FIC_I2C_INTR_ENABLED)
	mtv_cb_ptr->irq = mtv_confiture_irq();
#endif

	atomic_set(&mtv_cb_ptr->open_flag, 0);
	atomic_set(&mtv_cb_ptr->read_flag, 0);

	wake_lock_init(&mtv_cb_ptr->wake_lock, WAKE_LOCK_SUSPEND, RAONTV_DEV_NAME);
