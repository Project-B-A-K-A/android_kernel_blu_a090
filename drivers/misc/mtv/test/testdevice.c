...
{
	//DMSG0("test_delete_thread\n");
    if(tsp_read_thread_cb_ptr != NULL)
    {
        dmb_thread_run = 0;

		DMSG0("pthread_join tsp_read_thread_cb\n");
        pthread_join(tsp_read_thread_cb, NULL);
        tsp_read_thread_cb_ptr = NULL;
    }

	//DMSG0("mrevent_trigger\n");
    if(dm_timer_thread_cb_ptr != NULL)
    {
        dm_timer_thread_run = 0;
		DMSG0("mrevent_trigger begin\n");
        mrevent_trigger(&dm_timer.event);
		DMSG0("mrevent_trigger end\n");
...
APP_MAIN_EXIT:
	...
	EMSG0("test_delete_thread\n");
	test_delete_thread();

	EMSG0("close fd_dmb_dev\n");
    close(fd_dmb_dev);

#if defined(RTV_IF_TSIF) || defined(RTV_IF_SPI_SLAVE)
	EMSG0("close fd_testif_dev\n");
    close(fd_tsif_dev);
#endif
...
int main(int argc,char *argv[])
{
    int key, ret, iloop;
    char name[32];
    unsigned int dtv_ch_num = 69;
    unsigned int dtv_ch_freq = 0;
    unsigned int lock = 0;
    unsigned int unlock_counter = 0;
    unsigned int report_counter = 0;
    unsigned int find_one_ch = 0;
    ...
    for (iloop = 0; iloop < 3; iloop++) {
        if (iloop == 0){
            dtv_ch_num = 14; //{14, 473143}
            dtv_ch_freq = 473143;
        }
        else if (iloop == 1){
            dtv_ch_num = 43; //{43, 647143}
            dtv_ch_freq = 647143;
        }
        else {
            dtv_ch_num = 69; //{69, 803143}
            dtv_ch_freq = 803143;
        }

        //sleep(300);
        
        ret = ioctl(fd_dmb_dev, IOCTL_ISDBT_SCAN_FREQ, &dtv_ch_num);
        //EMSG1("IOCTL_ISDBT_SCAN_FREQ return %d\n", ret);
        if(ret == 0) {
            find_one_ch = 1;
            ...
            EMSG0("FAILED\n");
            EMSG0("IOCTL_ISDBT_SCAN_FREQ fail\n");
        }
    }
    ...
    while(find_one_ch)
    {
        ...
        //EMSG1("IOCTL_ISDBT_GET_SIGNAL_INFO return %d\n", ret);
        if(ret < 0)
        {
            EMSG0("FAILED\n");
        ...
        if (lock) {
            ...
            report_counter++;
            if (report_counter >= 2) {
                /*OK:FREQ:RSSI:CNR:BER*/
                printf("OK:%d:%.3f:%.3f:%.3f\n", dtv_ch_freq, 0.0/*(float)sig_info.rssi/RTV_ISDBT_RSSI_DIVIDER*/,
                            (float)sig_info.cnr/RTV_ISDBT_CNR_DIVIDER, (float)sig_info.ber/RTV_ISDBT_BER_DIVIDER);
                fflush(stdout);
                //break;
            }
        }
