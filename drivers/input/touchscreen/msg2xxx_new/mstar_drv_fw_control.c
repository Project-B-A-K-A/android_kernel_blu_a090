...
int mip_ges_trans_key(unsigned int code)
{
	struct file *fp; 
	...
	input_report_key(g_InputDevice, code, 1);
	input_sync(g_InputDevice);
	input_report_key(g_InputDevice, code, 0);
	input_sync(g_InputDevice);
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    DrvPlatformLyrSetIicDataRate(g_I2cClient, 100000); //50 KHz
}
...
#endif //CONFIG_TP_HAVE_KEY
////add  by jinq @20160409
#if defined(VIRTUAL_KEYS)
							if (tInfo.nTouchKeyCode == 4) // TOUCH_KEY_BACK
							{
								tInfo.tPoint[i].nX = 80; // final X coordinate
								tInfo.tPoint[i].nY = 1300; // final Y coordinate				
								DBG(&g_I2cClient->dev,"key touch released(tInfo.nTouchKeyCode == 4)\n");
			
							}
							else if (tInfo.nTouchKeyCode == 1) // TOUCH_KEY_MENU
							{
								tInfo.tPoint[i].nX = 400; // final X coordinate
								tInfo.tPoint[i].nY = 1300; // final Y coordinate
								DBG(&g_I2cClient->dev,"key touch released(tInfo.nTouchKeyCode == 1)\n");
			
							}			
							else if (tInfo.nTouchKeyCode == 2) // TOUCH_KEY_HOME
							{
								tInfo.tPoint[i].nX = 240; // final X coordinate
								tInfo.tPoint[i].nY = 1300; // final Y coordinate		 
								DBG(&g_I2cClient->dev,"key touch released(tInfo.nTouchKeyCode == 2)\n");
							...
							{	
								tInfo.tPoint[i].nX = 400; // final X coordinate
								tInfo.tPoint[i].nY = 1300; // final Y coordinate		   
								DBG(&g_I2cClient->dev,"key touch released(tInfo.nTouchKeyCode == 8)\n");
			
							}
						   // for (i = 0; i < tInfo.nFingerNum; i ++)
			
							if((tInfo.nTouchKeyCode == 8)||(tInfo.nTouchKeyCode == 2)||(tInfo.nTouchKeyCode == 1)||(tInfo.nTouchKeyCode == 4)) 
							{
