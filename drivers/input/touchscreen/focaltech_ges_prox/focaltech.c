...
#define  TPD_PROXIMITY_DEBUG_ON  0
...
#define TPD_PROXIMITY_DEBUG	ZYT_PRINTK
 ...
 struct Upgrade_Info fts_updateinfo[] =
{
    ...
    {0x64,"FT6x36U",TPD_MAX_POINTS_2,AUTO_CLB_NONEED,10, 10, 0x79, 0x18, 10, 2000},
...
static void check_gesture(int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_DOWN:
			...
			break;
#if !defined(CONFIG_FS251_CUST_IS_COLORS)
...
static int fts_read_Gestruedata(void)
{
	...
	pr_info( "tpd read FTS_GESTRUE_POINTS_HEADER.\n");
...
/*********************add by chenqx for TP shake when charging***************************/
	...
	for(i = 0; i < TS_MAX_FINGER; i++) {
		...
		if((buf[6*i+3] & 0x40) == 0x0) {
			...
			input_mt_sync(data->input_dev);
		#endif
			ZYT_PRINTK("=ft5x0x_update_data==x%d = %d,y%d = %d ====",i, x, i, y);
		}
		else {
		#if MULTI_PROTOCOL_TYPE_B
			input_mt_slot(data->input_dev, buf[6*i+5]>>4);
...
static int touch_event_handler(void *unused)
{
	...
	do {
		ZYT_PRINTK("=======touch_event_handler==========\n");
		...
		pr_info("====touch_event_handler== state = %d\n",state);
		if (state == 1)
		{
...
static void ft5x0x_ts_suspend(struct early_suspend *handler)
{
	...
	if (1==s_gesture_switch)
	{
		...
		ZYT_PRINTK("=====FTS_GESTRUE====>ft5x0x_ts_suspend, CHIP_ID = 0x%x. \n", fts_updateinfo_curr.CHIP_ID);
		if (fts_updateinfo_curr.CHIP_ID==0x54 || fts_updateinfo_curr.CHIP_ID == 0x64 )
		{
...
static int ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = check_ctp_chip();
	if (err<0) {
		pr_err("ft5x0x_ts_probe failed(%d): failed to check_ctp_chip\n", err);
