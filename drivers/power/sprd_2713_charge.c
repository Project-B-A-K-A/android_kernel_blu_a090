...
int s_temp_adc = 0,s_temp_vol = 0,s_temp_vol_avg = 0,s_bat_adc=0,s_bat_vol=0;
...
#define SAMPLE_NUM  15
	...
	if (pbat_data->temp_support) {
		...
		//ISENS mode
		if (pbat_data->temp_table_mode == 0) {
			ret = sci_adc_get_value_by_isen(pbat_data->temp_adc_ch,pbat_data->temp_adc_scale,30);
			s_temp_adc = ret;//@add by wangming for info
			return ret;
		}
		ret = sci_adc_get_values(&data);
		//ret = sci_adc_get_value_by_isen(pbat_data->temp_adc_ch,pbat_data->temp_adc_scale,3);
		s_temp_adc = ret;//@add by wangming for info
		WARN_ON(0 != ret);
...
int sprdchg_read_temp(void)
{
	if (pbat_data->temp_support) {
		...
		s_temp_vol = temp;//@wangming add for info
		...
		s_temp_vol_avg = temp;//@wangming add for info
		return temp;
	} else {
		return 200;
	}
...
{
	uint32_t voltage;
	s_bat_adc = sci_adc_get_value(ADC_CHANNEL_VBAT, false);
	voltage =
	    sprdchg_bat_adc_to_vol(s_bat_adc);
	s_bat_vol = voltage;
	return voltage;
}
