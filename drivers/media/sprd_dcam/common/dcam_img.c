...
LOCAL int sprd_img_setflash(uint32_t flash_mode, uint32_t flash_index)
{

	//printk("sprd_img_set_gpio_flash ======================%d;flash_index=%d\n",flash_mode,flash_index);
	switch (flash_mode) {
	...
	DCAM_TRACE("sprd_img_setflash: GPIO_flash_mode 0x%x  \n", flash_mode);
	return 0;
}
#else
LOCAL int sprd_img_setflash(uint32_t flash_mode, uint32_t flash_index)
{
	//printk("sprd_img_setflash ======================%d;flash_index=%d\n",flash_mode,flash_index);
	...
	DCAM_TRACE("sprd_img_setflash: flash_mode 0x%x index 0x%x \n", flash_mode, flash_index);
...
LOCAL int sprd_img_opt_flash(struct dcam_frame *frame, void* param)
{
	...
	if (info->flash_status < FLASH_STATUS_MAX) {
		DCAM_TRACE("SPRD_IMG: sprd_img_opt_flash, status %d index %d\n", info->flash_status, info->flash_index);
		...
		sprd_img_setflash(info->flash_status, info->flash_index);
	...
	while (1) {
		if (0 == down_interruptible(&dev->flash_thread_sem)) {
			if (dev->is_flash_thread_stop) {
				sprd_img_setflash(0, 0);
				sprd_img_setflash(0, 1);
...
static long sprd_img_k_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SPRD_IMG_IO_SET_FLASH:
		...
		dev->dcam_cxt.flash_status = flash_status & 0xFFFF;
#if defined(CONFIG_SUB_FLASH_PARALLEL_CONNECT) || defined(CONFIG_FRONT_FLASH_KPLED) || defined(CONFIG_CAMERA_FRONT_FLASH_COMMON)
		dev->dcam_cxt.flash_index = (flash_status >> 16);
#else
		dev->dcam_cxt.flash_index = 0;
#endif
		mutex_unlock(&dev->dcam_mutex);
		...
		DCAM_TRACE("SPRD_IMG: SPRD_IMG_IO_SET_FLASH, flash %d status %d\n", dev->dcam_cxt.flash_index, dev->dcam_cxt.flash_status);
		break;
