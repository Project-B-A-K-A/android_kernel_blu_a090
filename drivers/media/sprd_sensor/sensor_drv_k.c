...
LOCAL int _sensor_k_af_powerdown(int power_level)
{
	SENSOR_PRINT_HIGH("SENSOR: af pwdn %d\n", power_level);

//	gpio_direction_output(GPIO_SENSOR_AF_PWN, power_level);

	//gpio_set_value(GPIO_SENSOR_AF_PWN, power_level);

	return SENSOR_K_SUCCESS;
}
//@wangming 20150529 add zyt_info
struct sensor_name_tag s_zyt_info_sensor_name = {"Unknow","Unknow"};
				...
				unsigned long arg)
{
	...
	case SENSOR_IO_AF_PD:
		{    
			...
			ret = copy_from_user(&power_level, (int *) arg, sizeof(int));
			if (0 == ret) 
				ret = _sensor_k_af_powerdown(power_level);
